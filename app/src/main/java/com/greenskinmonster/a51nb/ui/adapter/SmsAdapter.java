...
import com.greenskinmonster.a51nb.ui.SmsFragment;
...
public class SmsAdapter extends BaseRvAdapter<SimpleListItemBean> {
    ...
    public void onBindViewHolderImpl(RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (item.getAuthorId().equals(HiSettingsHelper.getInstance().getUid())) {
            holder.iv_my_avatar.setTag(R.id.avatar_tag_uid, item.getAuthorId());
            holder.iv_my_avatar.setTag(R.id.avatar_tag_username, item.getAuthor());
        } else {
            holder.iv_friend_avatar.setTag(R.id.avatar_tag_uid, item.getAuthorId());
